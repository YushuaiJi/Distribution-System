...
- 每个servers会储存current term number，而且这个是单调递增的(这个单调递增非常重要）。
...
如果在选举过程中，candidate接受，来自别的servers要成为了leader的RPC。

如果leader的term（包含在RPC当中）>= candidate current term，那么这个leader是合法的。
